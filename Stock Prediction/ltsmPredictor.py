...
def predict_from_date_single_step(model, df, scaler, prediction_date, num_unrollings=60, days_ahead=60):
	...
	if len(actual_future_prices) > 0:
		...
		# Calculate unscaled (actual dollar) metrics
		mse_unscaled = mean_squared_error(actual_future_prices[:comparison_length], predictions_actual[:comparison_length])
		mae_unscaled = mean_absolute_error(actual_future_prices[:comparison_length], predictions_actual[:comparison_length])
		bias_unscaled = np.mean(predictions_actual[:comparison_length] - actual_future_prices[:comparison_length])
		
		# Calculate scaled (normalized) metrics
		actual_future_scaled = scaler.transform(actual_future_prices[:comparison_length].reshape(-1, 1)).flatten()
		predictions_scaled = scaler.transform(predictions_actual[:comparison_length].reshape(-1, 1)).flatten()
		mse_scaled = mean_squared_error(actual_future_scaled, predictions_scaled)
		mae_scaled = mean_absolute_error(actual_future_scaled, predictions_scaled)
		bias_scaled = np.mean(predictions_scaled - actual_future_scaled)
		
		print(f"Prediction bias (negative = underpredicting): ${bias_unscaled:.2f}")
		print(f"Scaled MSE: {mse_scaled:.6f}")
	else:
		mse_unscaled = mae_unscaled = bias_unscaled = None
		mse_scaled = mae_scaled = bias_scaled = None
...
def plot_date_prediction(prediction_results, df, prediction_date, train_data, scaler, ticker):
	"""
	...
	mse_scaled = prediction_results.get('mse_scaled', None)
	mae_scaled = prediction_results.get('mae_scaled', None)
	...
	title = f'{ticker} Predictions - Full Historical View'
	if mse_scaled is not None and mae is not None:
		title += f' (Scaled MSE: {mse_scaled:.6f}, MAE: ${mae:.2f}'
		if bias is not None:
	...
	historic_filename = f"{save_dir}/historic_{prediction_date.replace('-', '_')}.png"
	plt.savefig(historic_filename, dpi=300, bbox_inches='tight')
	print(f"Saved historic plot: {historic_filename}")
	plt.show()
	...
	detail_title = f'{ticker} Predictions - 60 Day Detailed View from {prediction_date}'
	if mse_scaled is not None and mae is not None:
		detail_title += f' (Scaled MSE: {mse_scaled:.6f}, MAE: ${mae:.2f}'
		if bias is not None:
	...
	# Save the 60-day detailed plot
	detail_filename = f"{save_dir}/60day_{prediction_date.replace('-', '_')}.png"
	plt.savefig(detail_filename, dpi=300, bbox_inches='tight')
	print(f"Saved 60-day plot: {detail_filename}")
	plt.show()
	...
	if len(actual_prices) > 0:
		...
		final_error = abs(pred_prices[-1] - actual_prices[-1])
		final_error_pct = (final_error / actual_prices[-1]) * 100
		print(f"Final Price Error: ${final_error:.2f} ({final_error_pct:.1f}%)")
		
		# Calculate total price movement
		total_actual_change = actual_prices[-1] - start_price
		total_predicted_change = pred_prices[-1] - start_price
		print(f"Actual 60-day change: ${total_actual_change:.2f} ({(total_actual_change/start_price)*100:.1f}%)")
		print(f"Predicted 60-day change: ${total_predicted_change:.2f} ({(total_predicted_change/start_price)*100:.1f}%)")
	
	if mse is not None:
		print(f"\n--- Performance Metrics ---")
		print(f"Unscaled MSE: {mse:.2f}")
		print(f"Unscaled MAE: {mae:.2f}")
		
	if mse_scaled is not None:
		print(f"Scaled MSE: {mse_scaled:.6f}")
		print(f"Scaled MAE: {mae_scaled:.6f}")
...
def run_prediction(ticker,
	...
	if prediction_date is not None:
		# Use rolling 1-year training window before prediction date
		pred_date = pd.to_datetime(prediction_date)
		training_start_date = pred_date - pd.Timedelta(days=365)
		
		print(f"Using rolling 1-year training window:")
		print(f"Training period: {training_start_date.strftime('%Y-%m-%d')} to {pred_date.strftime('%Y-%m-%d')}")
		
		# Filter data for training period
		training_mask = (df['Date'] >= training_start_date) & (df['Date'] <= pred_date)
		training_df = df[training_mask].copy()
		
		if len(training_df) < 100:  # Minimum data requirement
			raise ValueError(f"Insufficient training data. Only {len(training_df)} days available. Need at least 100 days.")
		
		print(f"Training data points: {len(training_df)} days")
		
		# Calculate mid prices for training period only
		print("\nStep 2: Calculating mid prices for training period...")
		high_prices = training_df['High'].values
		low_prices = training_df['Low'].values
		mid_prices = (high_prices + low_prices) / 2.0
		mid_prices = mid_prices.astype(np.float64)
		...
		print(f"Training data points: {len(mid_prices)}")

		print("\nStep 3: Scaling training data...")
		# Use all training data (no train/test split since we'll predict on future data)
		scaler = MinMaxScaler()
		mid_prices_scaled = scaler.fit_transform(mid_prices.reshape(-1, 1)).flatten()

		print("\nStep 4: Applying exponential smoothing...")
		train_data = apply_smoothing(mid_prices_scaled, gamma)
		...
		# Original approach - use all historical data
	...
	if prediction_date is not None:
		...
		print("(Model trained on 1-year rolling window)")
		...
		plot_date_prediction(pred_results, df, prediction_date, train_data, scaler, ticker)
		...
		results = {
			...
			'training_period': f"{training_start_date.strftime('%Y-%m-%d')} to {pred_date.strftime('%Y-%m-%d')}"
		}
...
results = run_prediction(
	ticker="BN",
	prediction_date="2025-04-15"  # Only the plot will focus on this date + 60 days
)
